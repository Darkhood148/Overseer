    ...
    def get_ai_overview(self, res: SerpResults) -> dict:
        ...
        if AI_OVERVIEW in res:
            if TEXT_BLOCKS in res[AI_OVERVIEW]:
                ...
                search_response = self.client.search(params)
                if (
                    AI_OVERVIEW in search_response
                    and TEXT_BLOCKS in search_response[AI_OVERVIEW]
                ):
...
class AIOverviewAnalyzer:
    ...
    def _process_references(self, references: list) -> None:
        ...
        for ref in references:
            if any(
                self.company in ref.get(field, "").lower()
    ...
    def check_occurrence(self) -> bool:
        ...
        """
        return (
            self.metrics.snippets > 0
            or self.metrics.headings.count > 0
            or self.metrics.references.count > 0
        )
...
def generate_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "-l", "--location", required=False, help="Optional location parameter"
    )
    parser.add_argument(
        "--metrics",
        action="store_true",
        help="Check how many times the company showed up in the AI Overview",
    )
    return parser
...
def main():
    ...
    if found:
        ...
        try:
            ...
            print(
                f"An error has occured while searching for the company in the results: {e}"
            )
            sys.exit(1)
